...
class ViTBCLangLearnAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        #for p in self._actor.parameters():
        #    #print(p.grad)
        #    if (not(p.data is None)) and torch.any(torch.isnan(p.data)):
        #        print("Data befores nan value has nan value", p.data)
        self._grad_step(loss, self._actor_optimizer,
                        self._actor.parameters(), self._grad_clip)
        #for name, p in self._actor.named_parameters():
        #    #print(p.grad)
        #    if (not(p.data is None)) and torch.any(torch.isnan(p.data)):
        #        print("Data has nan value", name, p.data)
        #exit()
        self._summaries = {
            'pi/loss': loss,
            'pi/mu': mu.mean(),
        ...
        #print("Same obervation before and after", mu, self._actor(observations, robot_state, lang_goal_desc))
        #exit()
